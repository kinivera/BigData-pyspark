...
popular_games_genre = df.alias("df1").join(popular_by_genre.alias("df2"), 
                        ...
                        .select(col("df1.Genres"), col("df1.Title"), col("df1.PopularityScore")).distinct()
...
df = df.withColumn("Team", explode(split("Team", ",")))\
       .withColumn("Team", regexp_replace(col("Team"), "\[|\]", ""))
...
popular_games_team = df.alias("df1").join(popular_by_team.alias("df2"), (col("df1.Team") == col("df2.Team")) & (col("df1.PopularityScore") == col("df2.MaxPopularityScore"))) \
                        .select(col("df1.Team"), col("df1.Title"), col("df1.PopularityScore")).distinct()
